...
public class AuthorizationController : Controller
{
        ...
        UserManager<ApplicationUser> userManager)
    {
        ...
        _openIddictApplicationClaimService = openIddictApplicationClaimService;
    }
    ...
    public async Task<IActionResult> Exchange()
    {
        ...
        else if (request.IsClientCredentialsGrantType())
        {
            ...
            identity.AddClaim(Claims.Role, OpenIddict.UI.Suite.Core.Roles.SuperAdmin);

            var applicationId =application?.GetType().GetProperty("Id")?.GetValue(application, null).ToString();

            var claims= await _openIddictApplicationClaimService.GetOpenIddictApplicationClaimAsync(applicationId);

            foreach (var item in claims)
            {
                identity.AddClaim(item.ClaimType, item.ClaimValue);
            }

            SetInternalClientClaims(identity, request);
    ...
    private void SetInternalClientClaims(ClaimsIdentity identity, OpenIddictRequest request)
    {
        if (request.ClientId ==_idpConfigurations.InternalClientId && request.ClientSecret == _idpConfigurations.InternalClientSecret)
        {
           var claims=  JsonConvert.DeserializeObject<List<OpenIddictApplicationClaim>>(request.Claims.ToString());

            foreach (var item in claims)
            {
                identity.AddClaim(item.ClaimType, item.ClaimValue);
            }
            var maxAge = request.GetParameter("maxage").ToString();
            if (double.TryParse(maxAge,out double max))
            {
                identity.SetAccessTokenLifetime(TimeSpan.FromSeconds(max));
            }
